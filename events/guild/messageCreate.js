...
module.exports = async (Discord, client, message) => {
    ...
    const prefix = await prefixCheck(message.guild.id);
    if(!message.content.startsWith(prefix)) {
        track.trackMessages(message);
        const words = await wordCheck(message.guild.id);
        if (words.length) {
    ...
    const blacklist = dbhelper.globalCache[message.guild.id].blacklist;
